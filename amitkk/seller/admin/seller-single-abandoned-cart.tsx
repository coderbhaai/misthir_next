...
import { CartProps, CartSkuProps } from '@amitkk/ecom/types/ecom';
    ...
    <Grid container spacing={4}>
      <Grid size={7}>
        ...
        <Box sx={{ display:'flex', alignItems: 'center'}}>
          <Box sx={{ mr: 3}}>
            <TextField fullWidth label="Email" sx={{ mb: 2 }} name="email" value="XXXXXXXX" disabled/>
            <FormControlLabel control={<Checkbox checked={Boolean(cart?.email)} disabled/>} label="Email me with news and offers" />
          </Box>
          <Box>
            <TextField fullWidth label="WhatsApp" sx={{ mb: 2 }} name="whatsapp" value="XXXXXXXX" disabled/>
        ...
        {cart?.shipping_address_id && "first_name" in cart.shipping_address_id && (
          <Typography variant="body2">{semiAddress(cart.shipping_address_id)}</Typography>
        )}
        ...
        <Grid container spacing={2} sx={{ mt: 2 }}>
          ...
          {cart?.billing_address_id && "first_name" in cart.billing_address_id && (
            <Typography variant="body2">{semiAddress(cart.billing_address_id)}</Typography>
          )}
                            ...
                            <ListItem key={item._id?.toString()} disablePadding sx={{ mb: 2 }}>
                              <Card sx={{ width: "100%", p: 1, alignItems: "center" }} elevation={0}>
                                ...
                                <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                                  ...
                                  <Button variant="outlined" size="small">+</Button>
                                  <Box sx={{ flexGrow: 1 }} />
                                  <Typography fontWeight="bold">₹{item.quantity * Number((item.sku_id as SkuProps)?.price ?? 0)}</Typography>
                                </Box>

                                <Button variant="outlined" color="primary" sx={{ display: "flex", flexDirection: "column", alignItems: "center", px: 2, py: 1, width: "100%", textTransform: "none", }} onClick={() => setSelectedSku(item)}>
                                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                    <DiscountIcon fontSize="small" />
                                    <Typography variant="body2" fontWeight="bold">Set Discount</Typography>
                                  </Box>
      ...
      <VendorDiscountModal
        ...
        cartSku={selectedSku}
        limit={Number((selectedSku.sku_id as SkuProps)?.price ?? 0)}
