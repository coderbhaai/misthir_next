...
import DataModal from "@amitkk/blog/components/author-modal";
import { AdminDataTable, DataProps } from "@amitkk/blog/components/admin-author-table";
...
export function AdminAuthor(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "blog/author?function=get_all_author");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `blog/author?function=get_single_author&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Authors" addButtonLabel="New Author" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminAuthor);