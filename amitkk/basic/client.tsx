...
import DataModal from "@amitkk/basic/components/client/client-modal";
import { AdminDataTable, DataProps } from "@amitkk/basic/components/client/admin-client-table";
...
export function AdminService(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "basic/basic?function=get_all_clients");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `basic/basic?function=get_single_client&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Clients" addButtonLabel="New Client" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
