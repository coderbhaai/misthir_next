...
import DataModal from "@amitkk/agency/admin/logo-portfolio-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-logo-portfolio-table";

export function AdminLogoPortfolio(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res_1 = await apiRequest("get", "agency/portfolio?function=get_all_logo_portfolios");
            setData(res_1?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/portfolio?function=get_single_logo_portfolio&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Logo Portfolio" addButtonLabel="New Logo Portfolio" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminLogoPortfolio);