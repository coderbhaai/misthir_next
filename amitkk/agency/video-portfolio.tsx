...
import DataModal from "@amitkk/agency/admin/video-portfolio-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-video-portfolio-table";

export function AdminVideoPortfolio(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "agency/portfolio?function=get_all_video_portfolios");
            setData(res?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/portfolio?function=get_single_video_portfolio&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Video Portfolio" addButtonLabel="New Video Portfolio" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminVideoPortfolio);