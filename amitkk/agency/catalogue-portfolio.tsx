...
import DataModal from "@amitkk/agency/admin/catalogue-portfolio-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-catalogue-portfolio-table";

export function AdminCataloguePortfolio(){
    ...
    const dataFiltered = useTableFilter<DataProps>( data, table.order, table.orderBy as keyof DataProps, filterData, ["heading"] );
    ...
    const fetchData = useCallback(async () => {
        try {
            const res_1 = await apiRequest("get", "agency/portfolio?function=get_all_catalogue_portfolios");
            setData(res_1?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/portfolio?function=get_single_catalogue_portfolio&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Catalogue Portfolio" addButtonLabel="New Catalogue Portfolio" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminCataloguePortfolio);