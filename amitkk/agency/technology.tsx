...
import DataModal from "@amitkk/agency/admin/technology-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-technology-table";
...
export function AdminTechnology(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "agency/technology?function=get_all_technology");
            setData(res?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/technology?function=get_single_technology&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Technologies" addButtonLabel="New Technology" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminTechnology);