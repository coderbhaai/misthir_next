...
import DataModal from "@amitkk/agency/admin/service-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-service-table";
...
export function AdminService(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "agency/service?function=get_all_services");
            setData(res?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/service?function=get_single_service&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Services" addButtonLabel="New Service" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
