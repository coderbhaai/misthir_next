...
import DataModal from "@amitkk/agency/admin/graphics-portfolio-modal";
import { AdminDataTable, DataProps } from "@amitkk/agency/admin/admin-graphics-portfolio-table";

export function AdminGraphicsPortfolio(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "agency/portfolio?function=get_all_graphics_portfolios");
            setData(res?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `agency/portfolio?function=get_single_graphics_portfolio&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Graphics Portfolio" addButtonLabel="New Graphics Portfolio" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminGraphicsPortfolio);