...
import { PortfolioProps, WebsitePortfolioProps } from '@amitkk/agency/types';
...
export interface DataProps extends PortfolioProps {
  function: string;
  selectedDataId: string | number | object | null;
  website_id?: WebsitePortfolioProps | string;
};
  ...
  return (
    <>
      <TableRow hover tabIndex={-1} role='checkbox' selected={selected}>
        ...
        <TableCell>{typeof row.website_id === "object" && "client_id" in row.website_id ? ( <UserRow row={row.website_id.client_id as unknown as UserRowProps} />) : null}</TableCell>
        <TableCell>{row.module}</TableCell>
        <TableCell>{typeof row.website_id === "object" && "client_id" in row.website_id ? ( <MediaImage media={row.website_id?.media_id as MediaProps}/> ) : null}</TableCell>
        <TableCell><StatusSwitch id={row._id.toString()} status={row.status} modelName="Portfolio"/></TableCell>
