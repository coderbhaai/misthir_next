...
import type {DataProps} from '@amitkk/agency/admin/admin-catalogue-portfolio-table';
...
export default function DataModal({ open, handleClose, selectedDataId, onUpdate, clientOptions }: DataFormProps) {
  const initialFormData: DataProps = {
    function : 'create_update_catalogue_portfolio',
    client_id: '',
    heading: '',
    content: '',
    status: true,
  ...
  React.useEffect(() => {
    if (open && selectedDataId) {
      const fetchData = async () => {
        try {
          const res = await apiRequest("get", `agency/portfolio?function=get_single_catalogue_portfolio&id=${selectedDataId}`);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formDataToSend.append("function", "create_update_catalogue_portfolio");
  ...
  const title = !selectedDataId ? 'Add Catalogue Portfolio' : 'Update Catalogue Portfolio';
