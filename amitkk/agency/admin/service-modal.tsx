...
import type {DataProps} from '@amitkk/agency/admin/admin-service-table';
...
export default function DataModal({ open, handleClose, selectedDataId, onUpdate }: DataFormProps) {
  ...
  React.useEffect(() => {
    if (open && selectedDataId) {
      const fetchData = async () => {
        try {
          const res = await apiRequest("get", `agency/service?function=get_single_service&id=${selectedDataId}`);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await apiRequest("post", `agency/service`, formDataToSend);
  ...
  const title = !selectedDataId ? 'Add Service' : 'Update Service';
