...
import type {DataProps} from '@amitkk/agency/admin/admin-portfolio-table';
...
type DataFormProps = TableDataFormProps & {
  ...
  websiteOptions: { _id: string; name: string }[];
};

export default function DataModal({ open, handleClose, selectedDataId, onUpdate, websiteOptions }: DataFormProps) {
  const initialFormData: DataProps = {
    function : 'create_update_portfolio',
    module: '',
    website_id: '',
    heading: '',
    status: true,
  ...
  React.useEffect(() => {
    if (open && selectedDataId) {
      const fetchData = async () => {
        try {
          const res = await apiRequest("get", `agency/portfolio?function=get_single_portfolio&id=${selectedDataId}`);
  
          setFormData({
            function: 'create_update_portfolio',
            module: res?.data?.module || "",
            website_id: res?.data?.website_id || "",
            heading: res?.data?.media_id || null,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formDataToSend.append("function", "create_update_portfolio");
      ...
      formDataToSend.append("website_id", formData.website_id?.toString() || "");
  ...
  const title = !selectedDataId ? 'Add Portfolio' : 'Update Portfolio';

  return (
    <CustomModal open={open} handleClose={handleCloseModal} title={title}>
      <form onSubmit={handleSubmit} style={{ maxHeight: "90vh", overflowY: "auto" }}>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, width: "100%" }}>
          <GenericSelect label="website" name="website_id" value={formData.website_id?.toString() ?? ""} options={websiteOptions} onChange={(val) => setFormData({ ...formData, website_id: val as string })}/>
