...
import { CataloguePortfolioProps } from '@amitkk/agency/types';
...
export interface DataProps extends CataloguePortfolioProps {
  function: string;
    ...
    <>
      <TableRow hover tabIndex={-1} role='checkbox' selected={selected}>
        { showCheckBox ? <TableCell padding='checkbox'><Checkbox disableRipple checked={selected} onChange={onSelectRow}/></TableCell> : null }
         <TableCell><UserRow row={row.client_id as unknown as UserRowProps}/><br/></TableCell>
        <TableCell><MediaImage media={row.media_id as MediaProps}/></TableCell>
        <TableCell><StatusSwitch id={row._id.toString()} status={row.status} modelName="CataloguePortfolio"/></TableCell>
