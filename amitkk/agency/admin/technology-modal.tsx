...
import type {DataProps} from '@amitkk/agency/admin/admin-technology-table';
...
export default function DataModal({ open, handleClose, selectedDataId, onUpdate }: DataFormProps) {
  const initialFormData: DataProps = {
    function : 'create_update_technology',
    name: '',
    url: '',
    title: '',
    description: '',
  ...
  React.useEffect(() => {
    if (open && selectedDataId) {
      const fetchData = async () => {
        try {
          const res = await apiRequest("get", `agency/technology?function=get_single_technology&id=${selectedDataId}`);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formDataToSend.append("function", "create_update_technology");
      formDataToSend.append("name", formData.name);
      formDataToSend.append("url", formData.url);
      formDataToSend.append("status", String(formData.status));
      formDataToSend.append("displayOrder", formData.displayOrder?.toString() || "0");
      formDataToSend.append("content", content);
      formDataToSend.append("path", "technology");

      formDataToSend.append("page_id", formData.page_id?.toString() || "");
      formDataToSend.append("meta_id", formData.meta_id?.toString() || "");
      ...
      const res = await apiRequest("post", `agency/technology`, formDataToSend);
  ...
  const title = !selectedDataId ? 'Add Technology' : 'Update Technology';
