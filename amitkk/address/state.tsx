...
export  function AdminState(){
    ...
    const updateData = async (i: StateProps) => { setUpdatedDataId(i?._id?.toString()); };
    const dataFiltered = useTableFilter<StateProps>( data, table.order, table.orderBy as keyof StateProps, filterData, ["name"] );
    const modalProps = { open, handleClose, selectedDataId, onUpdate: updateData };
    const handleEdit = (row: StateProps) => { setSelectedDataId(row._id.toString()); setOpen(true); };

    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "address/address?function=get_all_state");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `address/address?function=get_single_state&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<StateProps>
            title="States" addButtonLabel="New State" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminState);