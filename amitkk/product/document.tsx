...
export function AdminDocument(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "product/vendor?function=get_all_documents");
            setData(res?.data);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `product/vendor?function=get_single_document&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Documents" addButtonLabel="New Document" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminDocument);