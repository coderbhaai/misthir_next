...
export function AdminProductmeta(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "product/basic?function=get_all_product_metas");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `product/basic?function=get_single_product_meta&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Product Meta" addButtonLabel="New Product Meta" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminProductmeta);