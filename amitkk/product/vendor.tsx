...
import DataModal from "@amitkk/product/admin/vendor-modal";
import { AdminDataTable, DataProps } from "@amitkk/product/admin/admin-vendor-table";
...
export function AdminVendor(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "product/vendor?function=get_all_vendors");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `product/vendor?function=get_single_vendor&id=${updatedDataId}`);
                    const data = res?.data;
