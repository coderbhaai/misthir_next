...
export function AdminProductFeature(){
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await apiRequest("get", "product/basic?function=get_all_product_features");
            setData(res?.data ?? []);
    ...
    useEffect(() => {
        if (updatedDataId) {
            const fetchData = async () => {
                try {
                    const res = await apiRequest("get", `product/basic?function=get_single_product_feature&id=${updatedDataId}`);
                    const data = res?.data;
    ...
    return(
        <AdminTableLayout<DataProps>
            title="Features" addButtonLabel="New Feature" onAddNew={() => setOpen(true)} filterData={filterData} onFilterData={setFilterData} table={{ ...table, emptyRows: (totalRows: number) => emptyRows(table.page, table.rowsPerPage, totalRows)  }} data={dataFiltered}
            head={
...
export default withAuth(AdminProductFeature);